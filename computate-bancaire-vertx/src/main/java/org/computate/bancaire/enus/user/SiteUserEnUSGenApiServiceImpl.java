...
public class SiteUserEnUSGenApiServiceImpl implements SiteUserEnUSGenApiService {
						...
						if(b.succeeded()) {
							aSearchSiteUser(siteRequest, false, true, null, c -> {
								if(c.succeeded()) {
									...
									SimpleOrderedMap facets = (SimpleOrderedMap)listSiteUser.getQueryResponse().getResponse().get("facets");
	...
	public void sqlPATCHSiteUser(SiteUser o, Handler<AsyncResult<SiteUser>> eventHandler) {
		try {
			...
			patchSqlParams.addAll(Arrays.asList(pk, "org.computate.bancaire.enus.user.SiteUser"));
	...
	public void aSearchSiteUser(SiteRequestEnUS siteRequest, Boolean populate, Boolean store, String classApiUriMethod, Handler<AsyncResult<SearchList<SiteUser>>> eventHandler) {
		try {
			...
			listSearch.addFilterQuery("classCanonicalNames_indexed_strings:" + ClientUtils.escapeQueryChars("org.computate.bancaire.enus.user.SiteUser"));
