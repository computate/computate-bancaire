...
public class ClusterFrFRGenApiServiceImpl implements ClusterFrFRGenApiService {
						...
						if(b.succeeded()) {
							rechercheCluster(requeteSite, false, true, null, c -> {
								if(c.succeeded()) {
									...
									SimpleOrderedMap facets = (SimpleOrderedMap)listeCluster.getQueryResponse().getResponse().get("facets");
									...
									listePATCHCluster(listeCluster, dateStr, d -> {
										if(d.succeeded()) {
											SQLConnection connexionSql = requeteSite.getConnexionSql();
	...
	public void sqlPATCHCluster(Cluster o, Handler<AsyncResult<Cluster>> gestionnaireEvenements) {
		try {
			...
			patchSqlParams.addAll(Arrays.asList(pk, "org.computate.bancaire.frfr.cluster.Cluster"));
	...
	public void rechercheCluster(RequeteSiteFrFR requeteSite, Boolean peupler, Boolean stocker, String classeApiUriMethode, Handler<AsyncResult<ListeRecherche<Cluster>>> gestionnaireEvenements) {
		try {
			...
			listeRecherche.addFilterQuery("classeNomsCanoniques_indexed_strings:" + ClientUtils.escapeQueryChars("org.computate.bancaire.frfr.cluster.Cluster"));
